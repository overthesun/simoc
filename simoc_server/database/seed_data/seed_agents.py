...
def gen_human():
    ...
    data["human_min_arrival_age_attr"] = create_agent_type_attr(
            _type, "min_arrival_age", 21.0, units=age_units,
            description="Minimum expected arrival age of a human coming"
                "from earth")
    data["human_max_arrival_age_attr"] = create_agent_type_attr(
    ...
    data["human_max_energy_attr"] = create_agent_type_attr(
            _type, "max_energy", 21.0, units="MJ",
    ...
    metabolic_description_format = \
        "{} in metabolic function: (A - (age_factor*age(years)) + B(mass_factor*mass(kg) + height_factor*height(m)))/(C * work_factor * time)"
...
def add_plant(data, name, oxygen, carbon, water, edible, inedible):
    ...
    def add_plant_attribute(attribute_name, value, units, description):
