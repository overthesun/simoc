...
class AgentModel(Model):
    ...
    def create_new(cls, grid_width, grid_height):
        ...
        model.add_agent(greenhouse, (5,5))
        model.add_agent(crew_quarters, (0,0))

        atmosphere = cls.create_initial_atmosphere(model, [crew_quarters, greenhouse])
        plumbing_system = cls.create_initial_plumbing_system(model, [crew_quarters, greenhouse])
        storage = agents.StorageFacility(model, structure=greenhouse)
        model.add_agent(storage)
        harvester = agents.Harvester(model, structure=greenhouse)
        model.add_agent(harvester)
        planter = agents.Planter(model, structure=greenhouse)
        model.add_agent(planter)
        ...
        human_agent = agents.HumanAgent(model, structure=crew_quarters)
        model.add_agent(human_agent)
        plant_agent = agents.PlantAgent(model, structure=greenhouse)
