    ...
    function updateFinalize(){
        ...
        for(var i=0; i< configuration.length; i++){
            if(configuration[i].type =='hidden' ){
                document.getElementById('row-finalize').innerHTML += "<div class='col-6 final-right'>" + configuration[i].value + ':' + '</div>';
            }

            if(configuration[i].type =='select-one' || configuration[i].type == 'number'){
                document.getElementById('row-finalize').innerHTML += "<div class='col-6 final-left'>" + configuration[i].value + '</div>';
            }
            ...
            if(configuration[i].type =='radio' || configuration[i].type =='check'){
                if(configuration[i].checked){
                    ...
                    document.getElementById('row-finalize').innerHTML += "<div class='col-6 final-left'>" + document.getElementById(String(configuration[i].id)).value + '</div>';
                }
            }
